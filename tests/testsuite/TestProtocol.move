...
script {
    ...
    fun borrow_mai_a(sender: signer) {
        ...
        let amount = (200 * Token::scaling_factor<MAI::MAI>() as u128) ;
        STCVaultPoolA::borrow_mai(&sender, amount);
        let balance = Account::balance<MAI::MAI>(Signer::address_of(&sender));
...
script {
    ...
    fun borrow_mai(sender: signer) {
        ...
        let balance_before = Account::balance<MAI::MAI>(Signer::address_of(&sender));
        let amount = (80 * Token::scaling_factor<MAI::MAI>() as u128) + 1 ;
        STCVaultPoolA::borrow_mai(&sender, amount);
        let balance = Account::balance<MAI::MAI>(Signer::address_of(&sender));
        assert(amount + balance_before == balance, 1);
    }
...
script {
    ...
    fun borrow_mai(sender: signer) {
        ...
        assert(mai_debit == 1200 * Token::scaling_factor<MAI::MAI>(), 2);
...
script {
    ...
    fun repay_mai(sender: signer) {
        ...
        let a_market_cap = Token::market_cap<MAI::MAI>();
...
script {
    ...
    fun transfer_to_bob(sender: signer) {
        ...
        let amount = (200 * Token::scaling_factor<MAI::MAI>() as u128) ;
        STCVaultPoolA::borrow_mai(&sender, amount);
        let alice_balance = Account::balance<MAI::MAI>(Signer::address_of(&sender));
        assert(alice_balance == amount, 1);
        let bob_balance = Account::balance<MAI::MAI>(@bob);
        Account::pay_from<MAI::MAI>( & sender,@bob, amount / 2);
        let bob_balance_1 = Account::balance<MAI::MAI>(@bob);
...
use 0xb987F1aB0D7879b2aB421b98f96eFb44::MAI;
    ...
    fun repay_mai_all(sender: signer) {
        ...
        let after_balance = Account::balance<MAI::MAI>(Signer::address_of(&sender));
...
script {
    ...
    fun borrow_mai(sender: signer) {
        // current mock max is 1280
        let amount = (1280 * Token::scaling_factor<MAI::MAI>() as u128) + 1 ;
        STCVaultPoolA::borrow_mai(&sender, amount);
    }
}
// check: " Keep(ABORTED { code: 170503, "
...
script {
    ...
    fun borrow_mai(sender: signer) {
        ...
        assert(max == (1280 * Token::scaling_factor<MAI::MAI>() as u128),2);
    }
}
// check: "Keep(EXECUTED)"